  ...
  function loadATSBoard($root){
    ...
    function renderSavedViews(selectedId){
      ...
      viewCache.forEach(function(v){
        var name = v.name || 'View';
        if(defaultViewId && v.id === defaultViewId){ name = '★ ' + name; }
        html += '<option value="'+esc(v.id)+'"'+(v.id===selectedId?' selected':'')+'>'+esc(name)+'</option>';
      });
      $sel.html(html);
    }
    function loadSavedViews(selectedId){
      ajax('ai_suite_ats_saved_views_get', {}).done(function(res){
        if(!res || !res.ok){ return; }
        viewCache = res.views || [];
        defaultViewId = res.default_id || '';
        renderSavedViews(selectedId);
        if(defaultViewId && !selectedId){
          var v = null;
          viewCache.forEach(function(x){ if(x.id === defaultViewId){ v = x; } });
          if(v){
    ...
    $root.off('click.aisRenameView').on('click.aisRenameView', '#ais-ats-view-rename', function(){
      var id = String($root.find('#ais-ats-saved-views').val()||'');
      var name = String($root.find('#ais-ats-view-name').val()||'').trim();
      var $msg = $root.find('#ais-ats-view-msg');
      if(!id){ $msg.text('Selectează un view.'); return; }
      if(!name){ $msg.text('Introdu un nume pentru view.'); return; }
      ajax('ai_suite_ats_saved_views_rename', { id: id, name: name }).done(function(res){
        if(res && res.ok){
          viewCache = res.views || [];
          renderSavedViews(id);
          $msg.text('View redenumit.');
        } else {
          $msg.text((res && res.message) ? res.message : 'Eroare la redenumire.');
        }
      }).fail(function(){ $msg.text('Eroare la server (AJAX).'); });
    });

    $root.off('click.aisDefaultView').on('click.aisDefaultView', '#ais-ats-view-default', function(){
      var id = String($root.find('#ais-ats-saved-views').val()||'');
      var $msg = $root.find('#ais-ats-view-msg');
      if(!id){ $msg.text('Selectează un view.'); return; }
      ajax('ai_suite_ats_saved_views_default', { id: id }).done(function(res){
        if(res && res.ok){
          defaultViewId = res.default_id || id;
          renderSavedViews(id);
          $msg.text('Default setat.');
        } else {
          $msg.text((res && res.message) ? res.message : 'Eroare la setarea default.');
        }
      }).fail(function(){ $msg.text('Eroare la server (AJAX).'); });
    });
    ...
    $root.off('click.aisResetViews').on('click.aisResetViews', '#ais-ats-view-reset', function(){
      var $msg = $root.find('#ais-ats-view-msg');
      ajax('ai_suite_ats_saved_views_reset', {}).done(function(res){
        if(res && res.ok){
          viewCache = [];
          defaultViewId = '';
          renderSavedViews('');
          $root.find('#ais-ats-view-name').val('');
          $msg.text('Views resetate.');
        } else {
          $msg.text((res && res.message) ? res.message : 'Eroare la resetare.');
        }
      }).fail(function(){ $msg.text('Eroare la server (AJAX).'); });
    });
    ...
    $root.off('click.aisSmartItem').on('click.aisSmartItem', '.ais-smart-item', function(){
      ...
      if(type === 'company'){
        toast('info','ATS','Companie găsită: '+$(this).find('.ais-smart-title').text());
      }
    });
